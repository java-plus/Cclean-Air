...
public class QualiteAirService {
	...
	/**
	 * Méthode récupérant les données de la qualité de l'air de l'api pour chaque
	 * polluant et pour chaque station de mesure.
	 * 
	 * @return Map<String, List<Polluant>> : une map contenant pour chaque station
	 *         (key=le nom de la station), la liste des polluants (value).
	 */
	public Map<String, List<Polluant>> recupererDonneesQualiteAirDeApi() {
		...
		sbUrlDonneesPollution
				.append(formatter.format(ZonedDateTime.now().minusHours(1).withMinute(0).withSecond(0).withNano(0)));
		...
		for (Polluant p : polluants) {
			...
			try {
				...
				for (Mesure m : qualiteAirDtoApi.getMesures()) {
					Polluant polluant = new Polluant(m.getData().get(0).getUnite(), p.getNom(), p.getCode(),
							m.getData().get(0).getNiveau());
				...
				throw new QualiteAirInvalideException(
						"ERREUR : la récupération des données de l'API " + "de qualité d'air a échoué. " + "\n" + e);
	...
	/**
	 * Méthode sauvegardant la liste des polluants pour chaque station dans la table
	 * QualiteAir.
	 * 
	 * @param mapPolluants Map<String, List<Polluant>> : une map contenant pour
	 *                     chaque station (key=le nom de la station), la liste des
	 *                     polluants (value).
	 */
	public void sauvegarderQualiteAir(Map<String, List<Polluant>> mapPolluants) {
		mapPolluants.forEach((key, value) -> {
			QualiteAir qualiteAir = new QualiteAir(
					ZonedDateTime.now().minusHours(1).withMinute(0).withSecond(0).withNano(0), key);
	...
	/**
	 * Méthode retournant la date du dernier enregistrement dans la table de la
	 * qualité d'air.
	 * 
	...
	/**
	 * Méthode retournant la liste des QualiteAir de la date/heure indiquée de la
	 * base de données.
	 * 
	...
	/**
	 * Méthode retournant la QualiteAir dont la station était la plus proche des
	 ...
	 * @return [QualiteAir] la QualiteAir la plus proche géographiquement de la
	 *         commune.
	 */
	...
	/**
	 * Méthode permettant de supprimer de la base de données les données de qualité
	 * d'air antérieures à la date-heure indiquée.
	 * 
	...
	public List<QualiteAir> recupererLesQualitesAirAvantDate(ZonedDateTime dateHeureLimite)
			throws QualiteAirInvalideException {
		...
		try {
			...
			throw new QualiteAirInvalideException(
					"ERREUR : impossible de récupérer la liste des qualités d'air. \n" + e);
		}
	}
}
